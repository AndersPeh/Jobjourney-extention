...
function initializeUI(
  setupEventListenersCallback,
  loadSavedPreferencesCallback
) {
  ...
  if (missingElements.length > 0) {
    console.error(
      `Missing critical UI elements: ${missingElements.join(", ")}`
    );
...
function updateWebsiteOptions(elements, country) {
  ...
  if (country) {
    ...
    // JobStreet is available for Malaysia
    if (country === "Malaysia") {
      addWebsiteOption(
        websiteOptions,
        "jobstreet",
...
function showEmptyState(
  ...
  subMessage = "Try adjusting your search criteria or select different job platforms"
) {
...
function validateSearchForm(elements, showErrors = true) {
  const { searchInput, countrySelect, locationSelect, statusMessage } =
    elements;
  ...
  const checkboxes = document.querySelectorAll(
    '.website-option input[type="checkbox"]:checked'
  );
...
function showUpdateUI({ currentVersion, minimumVersion, message }) {
  ...
  description.innerHTML =
    message ||
    `Your extension version (${currentVersion}) is no longer compatible with JobJourney. Please update to version ${minimumVersion} or higher.`;
...
function showVersionOverlay(versionInfo) {
  ...
  messageElement.innerHTML =
    versionInfo.message ||
    "Your extension version is not compatible with the website.";
