  ...
  return new Promise(async (resolve) => {
    try {
      ...
      chrome.runtime.sendMessage({
        action: "SCRAPING_STATE_UPDATE",
        data: { tabId: tabId, isActive: true },
      });
      ...
      scrapingConfig.duration =
        scrapingConfig.endTime - scrapingConfig.startTime;
      ...
      chrome.runtime.sendMessage({
        action: "SCRAPING_STATE_UPDATE",
        data: { tabId: tabId, isActive: false },
      });
      try {
...
function createJobSearchUrls(searchTerm, location, platformFilter = null) {
  ...
  sites.push({
    id: "indeed",
    url: `https://${indeedDomain}/jobs?q=${encodeURIComponent(
      searchTerm
    )}&l=${encodeURIComponent(location.split(",")[0])}`,
  ...
  if (location.includes("Australia")) {
    sites.push({
      id: "seek",
      url: `https://www.seek.com.au/${seekSearchTerm}-jobs/in-${location
        .split(",")[0]
        .toLowerCase()
        .replace(/\s+/g, "-")}`,
      platform: "SEEK",
  ...
  // Add SEEK for New Zealand
    ...
    sites.push({
      id: "seek",
      url: `https://www.seek.co.nz/${seekSearchTerm}-jobs/in-${location
        .split(",")[0]
        .toLowerCase()
        .replace(/\s+/g, "-")}`,
      platform: "SEEK NZ",
  ...
  if (platformFilter) {
    ...
    console.log(
      `After filtering, found ${sites.length} URLs for`,
      platformFilter
    );
  }
