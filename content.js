...
function createScrapeOverlay() {
  ...
  subMessage.textContent =
    "Please do not close this tab or interact with the page until scraping is complete.";
...
function removeScrapeOverlay() {
  const overlay = document.getElementById(
    "jobjourney-scrape-overlay-container"
  );
  if (overlay) {
    overlay.remove();
  }
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  if (request.action === "scrapeJobs") {
    ...
    if (window.linkedInScraper && window.linkedInScraper.isMatch(currentUrl)) {
      ...
      window.indeedScraper &&
      window.indeedScraper.isMatch(currentUrl)
    ) {
      platformScraper = window.indeedScraper;
    } else if (
      window.jobstreetScraper &&
      window.jobstreetScraper.isMatch(currentUrl)
    ) {
      platformScraper = window.jobstreetScraper;
    }
...
function determinePlatform(url) {
  if (url.includes("linkedin.com")) {
    ...
    return "Indeed";
  } else if (url.includes("my.jobstreet.com")) {
    return "JobStreet";
  } else {
    return "Unknown";
  }
    ...
    chrome.runtime.sendMessage(
      ...
      (response) => {
        ...
        if (chrome.runtime.lastError) {
          console.warn(
            "Error sending VERSION_CHECK_RESPONSE to background:",
            chrome.runtime.lastError.message
          );
        } else if (response) {
          console.log(
            "Background acknowledged VERSION_CHECK_RESPONSE:",
            response
          );
        }
      }
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.action === "JOBS_SCRAPED") {
    ...
    if (sendResponse) {
      sendResponse({
        received: true,
        jobCount: message.data?.jobs?.length || 0,
      });
    }
  ...
  if (message.action === "DOWNLOAD_EXTENSION") {
    console.log(
      "Forwarding download extension request to website:",
      message.data
    );
